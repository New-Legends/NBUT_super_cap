...
void MX_FREERTOS_Init(void) {
  ...
  osThreadDef(communicate, communicate_task, osPriorityHigh, 0, 128);
  communicateHandle = osThreadCreate(osThread(communicate), NULL);

  /* definition and creation of cap_ctrl */
  osThreadDef(cap_ctrl, cap_ctrl_task, osPriorityHigh, 0, 128);
  cap_ctrlHandle = osThreadCreate(osThread(cap_ctrl), NULL);
